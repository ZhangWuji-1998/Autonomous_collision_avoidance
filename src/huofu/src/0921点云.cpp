...
{	
    ...
    //前面是z 左右是y 上下是x
    double depth_z = 0;
    double point_times=0;
	if(side_filted->points.size()>0){
		for(size_t i = 0; i < side_filted->points.size(); i++){
            if(!isnan(side_filted->points[i].x)&& !isnan(side_filted->points[i].y)&& !isnan(side_filted->points[i].z)){
                //std::cout<<sideway->points[i].x<<"\t"<<sideway->points[i].y<<"\t"<<sideway->points[i].z<<std::endl;
                
                depth_z = depth_z + side_filted->points[i].z;
                point_times++;
            }   
    	}
        depth = depth_z/point_times;
	}
