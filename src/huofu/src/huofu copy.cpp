...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber PointSub_ = nh.subscribe<sensor_msgs::LaserScan>(point_topic, 1, point_callback);
    //local_pos_pub = nh.advertise<geometry_msgs::PoseStamped>("/move_base_simple/goal", 1);
    local_pos_pub = nh.advertise<std_msgs::String>("/move_base_simple/goal", 1);
...
{	geometry_msgs::PointStamped camera;
	...
	// Vector4d p_w_q = exMatrix_ * p_c_q;
    // //实时获取相机外参
...
void point_callback(const sensor_msgs::LaserScan::ConstPtr& scan){
	...
	double depth_temp = 0;
	if(rawCloud.points.size()>0){
		for(size_t i = 0; i < rawCloud.points.size(); i++){
        // std::cout<<rawCloud.points[i].x<<"\t"<<rawCloud.points[i].y<<"\t"<<rawCloud.points[i].z<<std::endl;
		 depth_temp = depth_temp + rawCloud.points[i].x;	
    	}
		depth = depth_temp/rawCloud.points.size();
        //cout<<"depth_laser: "<<depth<<endl;
	}
}
